  ...
  <div>
    ...
    <template v-else-if="songs.length">
      <div class="detail-header">
        <div class="detail-cover circle">
          <img v-if="coverName" :src="`/media/covers/${coverName}`" alt="" />
          <span v-else>{{ name.charAt(0) }}</span>
        </div>
        <div class="detail-info">
          <h2>{{ name }}</h2>
          <div class="stats">{{ songs.length }} 首歌曲 · 累计播放 {{ totalPlays }} 次</div>
        </div>
      </div>
    ...
    <div v-else class="state">没有该歌手的歌曲</div>
  </div>
</template>
...
import { listSongsByArtist } from '../api'
...
const totalPlays = computed(() =>
  songs.value.reduce((sum, s) => sum + (s.playCount || 0), 0)
)
// 按播放次数降序排列（热度优先）
...
async function load() {
  loading.value = true
  try {
    songs.value = await listSongsByArtist(name.value)
